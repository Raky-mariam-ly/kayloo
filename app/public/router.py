...
@router.get("/agences/{agency_id}", response_class=HTMLResponse)
async def get_agency_profile(
    agency_id: UUID,
    request: Request,
    session: AsyncSession = Depends(get_db),
):
    agency = await AgencyRepository(session).get(agency_id)
    if agency is None or not agency.is_active:
        raise HTTPException(status_code=404, detail="Agence introuvable")

    properties = await PropertyRepository(session).get_by_agency(
        agency_id=agency_id,
        exclude_hidden=True,
    )
    return templates.TemplateResponse(request, "public/pages/partners/details.html", context={
        "agency": agency,
